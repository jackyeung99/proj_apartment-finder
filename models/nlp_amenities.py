...
class nlp_processor:
    def __init__(self, word2vec_model,input_list):
    ...
    def preprocess_text(self, text):
        ...
        lemmatized = [self.lemmatizer.lemmatize(token) for token in tokens if token not in stopwords.words('english') and token.isalpha()]
        return lemmatized

    def phrase_detection(self):
        tokenized_list = [self.preprocess_text(item) for item in self.input_list]
    ...
    def categorize_items(self):
        phrased_list = self.phrase_detection()
    ...
    def output_vector(self):
        ...
        category_len = {key: len(items) for key,items in categorized_items.items() if key != "Others"}
        items = np.array(category_len.values())
...
if __name__ == "__main__":
    ...
    nlp = nlp_processor(word2vec_model,test_list)
    nlp.main()
